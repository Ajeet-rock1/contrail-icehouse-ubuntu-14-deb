...
DEBIAN_FRONTEND=noninteractive dpkg -i binutils*.deb dpkg-dev*.deb libdpkg-perl*.deb make_*.deb patch*.deb python-pip*.deb python-pkg-resources*.deb python-setuptools*.deb python-colorama*.deb python-distlib*.deb python-html5lib*.deb libmysqlclient18*.deb mysql-common*.deb python-mysqldb*.deb
